...
read -p "Do you want to append path /opt/node-v18.16.0-linux-x64/bin/ to all rc files starting with a dot in your home directory? (y/n) " answer;
if [ "$answer" == "y" ]; then
  # 向~内的所有以点开头的rc文件写入path /opt/node-v18.16.0-linux-x64/bin/
